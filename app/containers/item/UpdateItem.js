...
function UpdateItem(props) {
    ...
    useEffect(() => {
        ...
        axiosInstance
            .get(`/items/${id}?company_id=${companyID}`, {
                headers: {
            ...
            .then((res) => {
                setDataState({
                    ...dataState,
                    generalName: res?.data?.data?.name,
                    categories: Number(res?.data?.data?.category_id),
                    description: res?.data?.data?.description,
                    purchasePrice: res?.data?.data?.purchase_price,
                    salePrice: res?.data?.data?.sale_price,
                    taxes: Number(res?.data?.data?.taxes?.data[0]?.tax_id)
                });
            })
            .catch((err) => {
                console.log(err);
    ...
    const submitForm = (e) => {
        e.preventDefault();
        axiosInstance
            .patch(`/items/${id}?company_id=${companyID}`, {
