...
function HistoryList(props) {
  ...
  const historyList = useSelector((state) => state.histories.histories);
  ...
  useEffect(() => {
    ...
    if (companyID !== '') {
      axiosInstance
        .get(`/histories?company_id=${companyID}&page=${currentPageNo}&limit=${10}`, {
          headers: {
            Authorization: localStorage.getItem('authorizationValue')
          }
        })
        .then((res) => {
          ...
          if (historyList.length === 0) {
            setHistories(res?.data?.data?.map((data) => [data.item_id, data.quantity, data.warehouse_id, data.type_id, data.id]));
            receivedData = res?.data?.data[0]
          }
          else {
            setHistories(res?.data?.data?.map((data) => [data.item_id, data.quantity, data.warehouse_id, data.type_id, data.id]));
            receivedData = historyList[0]
          }
          setData({
            "id": receivedData.id,
            "company_id": Number(companyID),
            "user_id": receivedData.user_id,
            "item_id": receivedData.item_id,
            "warehouse_id": receivedData.warehouse_id,
            "type_type": receivedData.type_type,
            "type_id": receivedData.type_id,
    ...
    axiosInstance
      ...
      .then((resData) => {
        axiosInstance
          ...
          .then((res) => {
            setAllItemsData(res?.data?.data?.map((data) => [data.name, data.category.name, [data.sale_price_formatted, data.purchase_price_formatted], data.id]));
            setTotalItems(res?.data?.meta?.total);
          })
          .catch((err) => {
            console.log(err);
  ...
  const names = [
    {
      id: 'itemID',
      name: 'Item ID',
      status: filterStatus.itemID
    },
    {
      id: 'quantity',
      name: 'Quantity',
      status: filterStatus.quantity
    },
    {
      id: 'warehouseID',
      name: 'Warehouse ID',
      status: filterStatus.warehouseID
    },
    {
      id: 'typeID',
      name: 'Type ID',
      status: filterStatus.typeID
    }
  ];
  ...
  const handleChangeColumnFilter = (event) => {
    ...
    if (value === "itemID") {
      setFilterStatus({
        itemID: !filterStatus.itemID,
        quantity: filterStatus.quantity,
        warehouseID: filterStatus.warehouseID,
        typeID: filterStatus.typeID
      })
    }
  ...
  const columns = [
    ...
    },
    {
      name: 'Warehouse ID',
      options: {
        filter: filterStatus.warehouseID,
        sort: false,
        display: columnVisibility,
      }
    },
  ...
  const handleRowClick = (rowData, rowMeta) => {
    ...
    var id = rowData[4].props.children
    var history_data = historyList.filter(history => {
      return history.id == id
    })
    var receivedData = history_data[0]
    setData({
      "id": receivedData.id,
      "company_id": Number(companyID),
      "user_id": receivedData.user_id,
...
const HistoryListMaped = connect(
  mapStateToProps,
  mapDispatchToProps
)(HistoryList);

export default (withStyles(styles)(HistoryListMaped));
