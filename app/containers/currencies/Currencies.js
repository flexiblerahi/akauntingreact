...
function Currencies(props) {
  ...
  useEffect(() => {
    ...
    if (companyID !== '') {
      axiosInstance
        .get(`/currencies?company_id=${companyID}&page=${currentPageNo}&limit=${rowsPerPages}`, {
          headers: {
            Authorization: localStorage.getItem('authorizationValue')
          }
        })
        .then((res) => {
          setAllCurrenciesData(res?.data?.data?.map((data) => [data.name, data.code, data.symbol, data.rate, data.id]));
  ...
  const handleDelete = (id) => {
    axiosInstance
      .delete(`/currencies/${id}?company_id=${companyID}`, {
        headers: {
          Authorization: localStorage.getItem('authorizationValue')
        }
      })
    ...
    },
    {
      ...
      options: {
        ...
        customBodyRender: (value) => <>
          <>
            <Link to={`/app/update/${value}/currencies/`}><Icon>edit</Icon></Link>
  ...
  return (
    <>
      <Link to={'/app/create/currencies/'}>New Currencies</Link>
