...
function Taxes(props) {
  ...
  useEffect(() => {
    ...
    axiosInstance
      .get(`/taxes?company_id=${companyID}&page=${currentPageNo}&limit=${rowsPerPages}`, {
        headers: {
          Authorization: localStorage.getItem('authorizationValue')
        }
      })
      .then((res) => {
        setAllTaxesData(res?.data?.data?.map((data) => [data.name, data.rate, data.id]));
  ...
  const handleDelete = (id) => {
    axiosInstance
      .delete(`/taxes/${id}?company_id=${companyID}`, {
        headers: {
          Authorization: localStorage.getItem('authorizationValue')
        }
      })
  ...
  const columns = [
    ...
    },
    {
      ...
      options: {
        ...
        customBodyRender: (value) => <>
          <>
            <Link to={`/app/update/${value}/tax/`}><Icon>edit</Icon></Link>
  ...
  return (
    <>
      <Link to={'/app/create/tax/'}>New Tax</Link>
