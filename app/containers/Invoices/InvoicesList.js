...
function InvoicesList(props) {
  ...
  useEffect(() => {
    ...
    if (companyID !== '') {
      axiosInstance
        .get(`/documents?company_id=${companyID}&page=${currentPageNo}&limit=${rowsPerPages}&search=type:invoice`, {
          headers: {
            Authorization: localStorage.getItem('authorizationValue')
          }
        })
        .then((res) => {
          dispatch(getinvoices(res?.data?.data));
          if (invoiceList.length === 0) {
            setInvoices(res?.data?.data?.map((data) => [[new Date(data.due_at).toLocaleDateString('en-US'), new Date(data.issued_at).toLocaleDateString('en-US')], data.status, [data.contact_name, data.document_number], data.amount, data.id]));
            data = res?.data?.data[0]
          }
          else {
            setInvoices(invoiceList?.map((data) => [[new Date(data.due_at).toLocaleDateString('en-US'), new Date(data.issued_at).toLocaleDateString('en-US')], data.status, [data.contact_name, data.document_number], data.amount, data.id]));
            data = invoiceList[0]
          }
          setInvoice(data)
          setData({
    ...
    axiosInstance
      ...
      .then((resData) => {
        axiosInstance
          ...
          .then((res) => {
            setInvoices(res?.data?.data?.map((data) => [[data.due_at, data.issued_at], data.status, [data.contact_name, data.document_number], data.amount, data.id]));
            setTotalItems(res?.data?.meta?.total);
            setRowsPerPages(res?.data?.meta?.per_page);
          })
          .catch((err) => {
  ...
  const handleRowClick = (rowData, rowMeta) => {
    ...
    var invoice_data = invoiceList.filter(invoice => {
      return invoice.id == id
    })
    var data = invoice_data[0]
    setInvoice(data)
    setData({
      company_id: Number(companyID),
      category_id: data.category_id,
              ...
              <Grid item container spacing={2}>
                ...
                <Grid md={6}>
                  ...
                  <Box display={'flex'} justifyContent={'end'} marginY={2}>
                    <Link className={customClasses.linkNew} to='/app/create/invoice'>Create Invoice</Link>
                    <Link className={customClasses.linkEdit} to={`/app/update/${invoice === null ? '' : invoice.id}/invoice`}>Edit</Link>
                  </Box>
                </Grid>
              </Grid>
              {invoice === null ? <></> : <Invoice data={data} handleChange={handleChange} />}
            </Box>
          </Box>
        </ThemeProvider>
      </div>
...
const InvoicesListMaped = connect(
  mapStateToProps,
  mapDispatchToProps
)(InvoicesList);

export default (withStyles(styles)(InvoicesListMaped));
